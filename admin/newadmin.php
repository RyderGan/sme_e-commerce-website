		...
		if (strlen($_POST['first_name']) > 2 && strlen($_POST['first_name']) < 16) {
			if ($email_check == 0) {
				if (strlen($_POST['password']) > 4) {
					...
					$stmt = mysqli_prepare($con, "INSERT INTO admin (firstName,lastName,email,mobile,address,password,type, confirmCode) VALUES (?,?,?,?,?,?,?,?)");
					mysqli_stmt_bind_param($stmt, "ssssssss", $u_fname, $u_lname, $u_email, $u_mobile, $u_address, $hashed_password, $u_admintype, $confirmCode); // exaple code
					mysqli_stmt_execute($stmt);
					mysqli_stmt_close($stmt);

					// success message
					$success_message = '<div class="signupform_content"><h2><font face="bookman">Admin Registration Successful!</font></h2>
					...
					throw new Exception('Password must be 5 or more characters!');
				}
			} else {
...
<body class="home-welcome-text" style="background-image: url(../image/homebackgrndimg2.png);">
	...
	<?php if (isset($success_message)) : ?>
		<p><?= $success_message ?></p>
	<?php elseif (isset($error_message) || (!isset($error_message) && !isset($success_message))) : ?>
